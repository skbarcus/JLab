...
class DVCS_Pulser_Control_GUI:
    def __init__(self, primary):
        ...
        range_label_text.set("To operate the DVCS pulser first have CODA open and configured on the machine running this GUI. Then download the library ")
...
def toggle_power(button,buttons,indicator_lights,voltage_setting_entries,readback_labels,predicted_threshold_labels):
    ...
    if in_range == True:
        ...
        os.system('remex ROC17 \'mpv904SetDAC(0,'+str(ch)+','+str(set_value)+')\'')
...
def load_library():
    ...
    os.system('remex -d ROC17 linuxvme/dvcsPulser/libdvcsPulser.so')

def initialize():
    #Initializes the DVCS pulser.
    os.system('remex ROC17 \'dvcsPulserInit(0xEF0,0)\'')

def enable_chs():
    #Enables the output channels for the DVCS pulser.
    os.system('remex ROC17 \'dvcsPulserEnable(0x1d)\'')
