...
#include "SBSEArm.h"
#include "SBSTestDetector.h"

void replay_test_detector(Int_t runnum = 495, Int_t lastEvent = -1){

  //gSystem->Load("../sbsoffline/build_a17/libsbs.so");
  SBSTestDetector *test_detector = new SBSTestDetector("test_detector","TestDetector");
  ...
  SBSEArm *harm = new SBSEArm("sbs","Hadron Arm with TestDetector");
  ...
  while(seg_ok) {
    ...
    analyzer->SetOutFile( TString::Format("%s/fadc_f1tdc_test_detector_%d.root",
      getenv("HCAL_ROOTFILES"),runnum));
    // File to record cuts accounting information
    analyzer->SetSummaryFile(TString::Format("fadc_f1tdc_test_detector_%d.log",runnum)); // optional
