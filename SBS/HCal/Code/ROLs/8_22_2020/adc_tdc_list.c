...
#endif
/*#include "remexLib.h"*/
#include "hcal_usrstrutils.c"

#ifdef ENABLE_HCAL_PULSER
#include "hcalLi(islot = 0; islot < NF1TDC; islot++) {b.h"0
#endif


#ifdef ENABLE_FADC
/* FADC250 Global Definitions */
#define FADC_START_SLOT       18    /* First ADC SLOT */
int faMode=1;
...
//#define FADC_WINDOW_LAT       200  /* Trigger Window Latency -> Now read in flags file.*/
//#define FADC_WINDOW_WIDTH     200  /* Trigger Window Width -> Now read in flags file*/
/* New values for LED pulser 2019-06-01 */
//#define FADC_WINDOW_LAT       180  /* Trigger Window Latency -> Now read in flags file.*/
//#define FADC_WINDOW_WIDTH      80  /* Trigger Window Width -> Now read in flags file. */
/* 2019-06-18 */
...
/* 2020-09-29: SD_ADDR no longer needed. Using VXS SD module */
//#define FADC_SD_ADDR         0xEA00 /* Address of the FADC SD */

/* Settings for cosmic tests 10/7/2019 */
/* 2020-09-29: now read from flags file */
//#define FADC_WINDOW_LAT       7  //20 for half detector. /* Trigger Window Latency */
...
int NF1TDC = 5;
/* Settings for cosmic tests 3/10/2020 */
#define F1_WINDOW 300
#define F1_LATENCY 300
#endif
...
//#endif
  ...
  */
  ...
  /* Needs to be in prestart (which it is), but the examples
     also shows it here. So we copied it here too. */
...
/*   tiSetup(21); */
  ...
  /*  NFADC = (16+2-4)*0+0;*/   /* 16 slots + 2 (for the switch slots) */
  FA_SLOT=FADC_START_SLOT;
  if(FA_SLOT==12) { // Skip slot 12! Slot 11 will be skipped in the logic of the foor loop
    FA_SLOT++;
  }
  ...
  fadcA32Base=0x0B000000;       /*Previously 0x09000000, but conflicted with F1 A32 address when 5+ F1s. */
...
//#ifndef DISABLE_MULTIBLOCK_READOUT
      roflag=1; // Set to 1 by default, but in the case of multiple FADCs set to 2
...
//#endif
	...
	if(faGetBlockError(0) != FA_BLOCKERROR_NO_ERROR)
	  {
	    printf("    intCount = %d   CSR 19: 0x%08x (0x%08x)  20: 0x%08x (0x%08x)\n",
		   tiGetIntCount(),
		   faReadCSR(19), csr19, faReadCSR(18), csr18);
	  }
      }
      BANKCLOSE;
      vmeDmaConfig(2,3,0);
